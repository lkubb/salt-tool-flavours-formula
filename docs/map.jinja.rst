...
We will see a quick setup to configure the ``tool_flavours`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_flavours`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_flavours`` formulas under ``/srv/salt/tool_flavours/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_flavours/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_flavours/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_flavours/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_flavours/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_flavours`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_flavours`` formulas under ``/srv/salt/tool_flavours/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_flavours/parameters/roles

We will define 2 roles:

- ``tool_flavours/server``
- ``tool_flavours/client``

We create a configuration for the role ``tool_flavours/server`` in ``/srv/salt/tool_flavours/parameters/roles/tool_flavours/server.yaml``:
...
We create another configuration for the role ``tool_flavours/client`` in ``/srv/salt/tool_flavours/parameters/roles/tool_flavours/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_flavours/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_flavours/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_flavours/parameters/dns:domain/  '/srv/salt/tool_flavours/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-flavours-as-json:
      test.nop:
        - name: {{ flavours | json }}
...
.. code-block:: sls
    ...
    flavours-config-file-file-managed:
      file.managed:
        - name: {{ flavours.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='flavours-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ flavours.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
          - sls: {{ sls_package_install }}
        - context:
            flavours: {{ flavours | json }}

This ``sls`` file expose a ``flavours`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ flavours['winner'] }}
